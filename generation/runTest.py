...
path = os.getcwd() + "/Assignment1-Algo"
pathToGeneration = path + "/generation/"
...
def createFile(size, density):
    f = open(pathToGeneration + 'Data/'+ str(size) + "-" + str(density) + '.txt', 'w')
    for i in range(size):
...
def generateCommands(numCommands, size):
    f = open(pathToGeneration + 'testCommands.in', 'w')
...
if __name__ == '__main__':
        ...
        for l in range(len(typeList)):
            ...
            for k in range(len(densityList)):
                ...
                for i in range(len(sizeList)):
                    ...
                    dataString = pathToGeneration + "Data/" + str(str(sizeList[i])) + "-" + str(densityList[k]) + '.txt'
                        ...
                        for x in range(0, 1000):
                            ...
                            try:
                                stdout = subprocess.run(["python", path +  "/spreadsheetFilebasedTesting.py", 
                                                    ...
                                                    pathToGeneration + "testCommands.in", 
                                                    path + "/sample.out"], 
    ...
    with open(pathToGeneration + 'time.txt', 'w') as f:
